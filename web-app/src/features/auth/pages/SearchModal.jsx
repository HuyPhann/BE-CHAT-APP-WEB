...
const SearchModal = ({ onClose }) => {
  ...
  const fetchCurrentUser = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/users/id/${currentUserId}`
      );
    ...
    try {
      ...
      try {
        const sentRequestsResponse = await axios.get(
          `http://localhost:5000/api/friends/get-add-friend/${userId}`
        );
  ...
  const searchUser = debounce(async (query) => {
    if (query.trim()) {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5000/api/users/phone/${query}`
        );
  ...
  const handleViewRecentProfile = async (search) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/api/users/phone/${search.username}`
      );
      const userData = response.data;
