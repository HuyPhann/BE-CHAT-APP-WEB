      ...
      const fetchFriends = async () => {
        try {
          const response = await axios.get(
            `http://localhost:5000/api/friends/get-friend/${currentUserId}`
          );
...
const MemberDetailsModal = ({
  ...
  useEffect(() => {
    if (isOpen && member?._id) {
      const fetchMemberDetails = async () => {
        ...
        try {
          const response = await axios.get(
            `http://localhost:5000/api/users/id/${member._id}`
          );
  ...
  const handleTransferAdmin = async () => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.post(
          "http://localhost:5000/api/groups/change-admin",
          {
            groupId,
...
const GroupInfoPanel = ({
  ...
  const handleChangeAvatar = async (event) => {
    ...
    try {
      ...
      const response = await axios.put(
        "http://localhost:5000/api/groups/update-avatar",
        formData,
        {
          headers: {
  ...
  const handleRenameGroup = async () => {
    ...
    if (result.isConfirmed) {
      try {
        setRenaming(true);
        const response = await axios.post(
          "http://localhost:5000/api/groups/rename-group",
          {
            groupId: groupInfo.id,
    ...
    const fetchData = async () => {
      ...
      try {
        const endpoint = isGroup
          ? `http://localhost:5000/api/groups/id/${groupInfo.id}`
          : `http://localhost:5000/api/users/id/${groupInfo.id}`;
  ...
  const handleLeaveGroup = async (groupId, memberId) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.post(
          "http://localhost:5000/api/groups/leave-group",
          {
            groupId,
            memberId,
          }
  ...
  const handleDeleteAllMessages = async (
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.post(
          "http://localhost:5000/api/messages/delete-all-messages-for-me",
          requestBody
        );
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.delete(
          "http://localhost:5000/api/groups/delete-group",
          {
            data: {
              groupId,
              memberId: adminId,
